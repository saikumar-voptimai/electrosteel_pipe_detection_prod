...
class ROIWizardConfig:
    ...
    window_name: str = (
        "ROI Redraw Wizard - q=quit, u=undo, n=next, c=clear, ENTER=submit l=load existing roi for edit"
    )
...
class ROIRedrawWizard:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if key == ord("q"):  # quit
                ...
                raise RuntimeError(
                    "ROI redraw cancelled by user (q). No changes saved."
                )
            ...
            if key == ord("l"):  # load existing for edit
                spec = self.specs[self._idx]

                if not self._load_existing_for_edit(spec):
                    logger.warning(f"No existing ROI. You must draw manually.")
    ...
    def save(self) -> None:
        ...
        payload = {
            name: [[int(x), int(y)] for (x, y) in pts] for name, pts in rois.items()
        }
    ...
    def _capture_first_frame(self) -> np.ndarray:
        ...
        if not cap.isOpened():
            raise RuntimeError(
                f"Cannot open video source: {self.cfg.video_source}")
    ...
    def _set_display_frame(self, frame_orig: np.ndarray) -> None:
        ...
        if scale < 1.0:
            new_w = int(w * scale)
            new_h = int(h * scale)
            self._frame_disp = cv2.resize(
                frame_orig, (new_w, new_h), interpolation=cv2.INTER_AREA
            )
        else:
    ...
    def _load_existing_for_edit(self, spec):
        if spec.name in self._existing_rois:
            ...
            # Load into CURRENT editable points
            self._current_points_disp = pts_disp.copy()

            logger.info(
                f"[EDIT MODE] {spec.name} → loaded existing ROI for editing")
            return True
        else:
            logger.warning(f"[NO EXISTING] {spec.name}")
            return False
    ...
    def _render(self) -> np.ndarray:
        ...
        # Blend overlay
        cv2.addWeighted(
            overlay, self.cfg.alpha_fill, base, 1.0 - self.cfg.alpha_fill, 0, base
        )
        ...
        for spec in self.specs:
            ...
            cv2.polylines(
                ...
                thickness=self.cfg.edge_thickness,
            )
        ...
        if self._idx < len(self.specs):
            ...
            for px, py in pts:
                cv2.circle(base, (px, py), self.cfg.point_radius,
                           spec.color_bgr, -1)
    ...
    def _draw_help_text(self, img: np.ndarray, spec: ROISpec) -> None:
        ...
        lines = [
            ...
            spec.help_text,
            "",
            "Controls: LeftClick=point | u=undo | c=clear | ENTER=accept (needs 4 points) | l=load existing roi| q=quit | n=next",
        ]
        y = 30
        for line in lines:
            cv2.putText(
                img, line, (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2
            )
            y += 28
...
def default_roi_specs() -> List[ROISpec]:
    ...
    return [
        ROISpec(
            RoiName.LOADCELL.value,
            (0, 255, 255),
            "Loadcell zone: trigger PLC when ELIGIBLE pipe enters this ROI.",
        ),
        ROISpec(
            RoiName.CASTER_ORIGIN.value,
            (255, 255, 0),
            "Caster origin zone: includes, caster, trolley-end-area upto gate1.",
        ),
        ROISpec(
            RoiName.LEFT_ORIGIN.value,
            (0, 165, 255),
            "Left origin/exclusion: pipes here are ignored. If originating here origin='other'.",
        ),
        ROISpec(
            RoiName.RIGHT_ORIGIN.value,
            (255, 0, 255),
            "Right origin/exclusion: pipes here are ignored. If originating here origin='other'.",
        ),
        ROISpec(
            RoiName.GATE1_CLOSED.value,
            (255, 0, 0),
            "Gate1 closed reference ROI (4-pt box). Used as baseline for geometry checks.",
        ),
        ROISpec(
            RoiName.GATE1_OPEN.value,
            (0, 255, 0),
            "Gate1 open ROI tall-narrow: gate should land here when open.",
        ),
        ROISpec(
            RoiName.GATE2_CLOSED.value,
            (128, 0, 0),
            "Gate2 closed reference ROI (4-pt box). Used as baseline for geometry checks.",
        ),
        ROISpec(
            RoiName.GATE2_OPEN.value,
            (0, 128, 0),
            "Gate2 open ROI tall-narrow: gate should land here when open.",
        ),
        ROISpec(
            RoiName.SAFETY_CRITICAL.value,
            (0, 0, 255),
            "Safety zone: human detection inside this ROI is flagged/used for occlusion checks.",
        ),
    ]
